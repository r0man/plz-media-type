...
(ert-deftest test-plz-media-type-request:application/x-ndjson:ollama-blank-lines ()
  (plz-media-type-test-with-mock-response (plz-media-type-test-response "application/x-ndjson/ollama-blank-lines.txt")
    (let* ((else) (finally) (then) (objects)
           (process (plz-media-type-request 'get "MOCK-URL"
                      :as `(media-types ((application/x-ndjson
                                          . ,(plz-media-type:application/x-ndjson
                                              :handler (lambda (object) (push object objects))))))
                      :else (lambda (object) (push object else))
                      :finally (lambda () (push t finally))
                      :then (lambda (object) (push object then)))))
      (plz-media-type-test-wait process)
...
(ert-deftest test-plz-media-type-request:application/x-ndjson:ollama-broken ()
  (plz-media-type-test-with-mock-response (plz-media-type-test-response "application/x-ndjson/ollama-broken.txt")
    (let* ((else) (finally) (then) (objects)
           (process (plz-media-type-request 'get "MOCK-URL"
                      :as `(media-types ((application/x-ndjson
                                          . ,(plz-media-type:application/x-ndjson
      ...
      (let ((error (car else)))
        (should (equal "Failed to parse response, 31 bytes unprocessed" (plz-error-message error)))
        (let ((response (plz-error-response error)))
          (should (equal 200 (plz-response-status response)))
          (should (equal "{\"model\":\"llama2\",\"created_at\":" (plz-response-body response)))))
      (should (equal '(t) finally))
      (should (null then))
      (seq-doseq (response then)
        (should (plz-response-p response))
...
(ert-deftest test-plz-media-type-request:application/x-ndjson:proxy-http ()
  (plz-media-type-test-with-mock-response (plz-media-type-test-response "application/x-ndjson/proxy-http.txt")
    (let* ((else) (finally) (then) (objects)
           (process (plz-media-type-request 'get "MOCK-URL"
                      :as `(media-types ((application/json
                                          . ,(plz-media-type:application/x-ndjson
      ...
      (should (equal 5 (length objects)))
      (should (equal '((url . "http://httpbin.org/stream/5")
                       (args)
                       (headers
                        (Host . "httpbin.org")
                        (X-Amzn-Trace-Id . "Root=1-6662da41-6fd34cf116cc64ea50c16c0b")
                        (User-Agent . "curl/7.81.0")
                        (Accept . "*/*")
                        (Accept-Encoding . "deflate, gzip, br, zstd"))
                       (origin . "80.187.69.180")
                       (id . 0))
                     (seq-elt objects 4)))
      (should (equal '((url . "http://httpbin.org/stream/5")
                       (args)
...
(ert-deftest test-plz-media-type-request:application/x-ndjson:proxy-https ()
  (plz-media-type-test-with-mock-response (plz-media-type-test-response "application/x-ndjson/proxy-https.txt")
    (let* ((else) (finally) (then) (objects)
           (process (plz-media-type-request 'get "MOCK-URL"
                      :as `(media-types ((application/json
                                          . ,(plz-media-type:application/x-ndjson
      ...
      (seq-doseq (response then)
        ...
        (should (equal 200 (plz-response-status response)))
        (should (null (plz-response-body response)))
        (should (equal '((date . "Fri, 07 Jun 2024 10:00:22 GMT")
                         (content-type . "application/json")
                         (server . "gunicorn/19.9.0")
                         (access-control-allow-origin . "*")
                         (access-control-allow-credentials . "true"))
                       (plz-response-headers response))))
      (should (equal 5 (length objects)))
      (should (equal '((url . "https://httpbin.org/stream/5")
                       (args)
                       (headers
                        (Host . "httpbin.org")
                        (X-Amzn-Trace-Id . "Root=1-6662da36-524b2c8b4564906800823fce")
                        (User-Agent . "curl/7.81.0")
                        (Accept . "*/*")
                        (Accept-Encoding . "deflate, gzip, br, zstd"))
                       (origin . "80.187.69.180")
